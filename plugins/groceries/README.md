...
This directory is a standalone plugin (`plugin.json` at root). Install the path `plugins/groceries` in any Agent Plugins v1 client (Cursor, VS Code, Claude Code, etc.).

## What it provides

- **MCP**: `groceries` → `streamable-http` `https://ai-meal-planner-mcp.aranlucas.workers.dev/mcp` (OAuth)
